...
<div align="center">
  ...
  <div align="left">GIを考慮しない場合では，光が届かないため直方体の側面が暗く見える(左)．GIを考慮することで，壁からの反射光により直方体の側面が照らされて明るく見える(右)．<div>
</div>
...
<div align="center">
  ...
  <div align="left">ナイーブなパストレーシングでは，光源までの経路を効率よくサンプルできずノイズが生じる(左)．明示的に光源サンプリングを行うことで，効率よく経路構築が行える(右)．<div>
</div>

### 重点的サンプリング

効率よくレイをサンプリングするために，高い寄与が期待される箇所にサンプルを集中させる重点的サンプリング(IS: Importance Sampling)も実装しています．図4は表面粗さの異なるプレート(上に行くほど滑らかになる)と大きさの異なる光源が含まれるシーンの表示結果です(シーン出典: https://pbrt.org/scenes-v3)．
...
<div align="center">
  ...
  <br>図４ 重点的サンプリングを用いた表示結果<br>
  <div align="left">BRDFに基づくISでは，大きい光源や滑らかなプレートで上手くサンプリングできるが，粗いプレートでは上手くサンプリングができない(左)．光源に基づくISでは小さい光源や粗いプレートでは上手くサンプリングできるが，大きい光源を上手くサンプリングできない(右)．<div>  
</div>
